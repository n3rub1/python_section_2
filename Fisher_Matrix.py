...
def _calculateInnerFormula(sigmaX, sigmaY, sigmaXY):
    ...
    innerPart = (((sigmaX - sigmaY)/4)+(sigmaX * sigmaY))
...
def _calculateTanTwotheta(sigmaX, sigmaY, sigmaXY):
    ...
    return ((2*sigmaX*sigmaY)/(sigmaX-sigmaY))
...
def ellipse_params(fisherMatrix, levelOfConfidence):
    ...
    result =  np.array([_calculateASquared(fisherMatrix[0][0], fisherMatrix[1][1], fisherMatrix[0][1], levelOfConfidence)
...
fig, ax = plt.subplots()
ellipse = Ellipse(xy=(0, 0), width=ellipseTauVsW_1[0], height=ellipseTauVsW_1[1], angle=ellipseTauVsW_1[2], fill=False, color= "blue")
ax.add_artist(ellipse)
plt.xlim(-ellipseTauVsW_1[0], ellipseTauVsW_1[0])
plt.ylim(-ellipseTauVsW_1[1], ellipseTauVsW_1[1])
plt.show()
